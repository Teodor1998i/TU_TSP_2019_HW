    ...
    public partial class Form1 : Form
    {
        ...
        void calcSin() {
            try
            {
                ...
                if (num <= 360 && num >= -360)
                {
                    ...
                    MessageBox.Show("Моля, въведете коректни данни за първото поле: [-360:360]");
                }
            }
            catch {
                MessageBox.Show("Моля, въведете коректни данни за първото поле: [-360:360]");
        ...
        void calcRad() {
            ...
            try
            {
                if (num > 0 && num < 11)
                {
                    string msg = "Лице и обиколка на окръжност: \n";
                    msg += "Лицето е: " + Math.Round(num*num*Math.PI, 2).ToString() + "\n";
                    msg += "Обиколката е: " + Math.Round(2* Math.PI * num, 2).ToString() + "\n";

                    MessageBox.Show(msg);
                }
                else {
                    MessageBox.Show("Моля, въведете коректни данни за второто поле: [1-10]");
                }
            }
            catch
            {
